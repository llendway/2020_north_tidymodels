...
Adjust the workflow to include the best tuning parameter (smallest rmse, in this case) in the model. There are other ways you can select models, like `select_by_one_std_error()` which "selects the most simple model that is within one standard error of the numerically optimal results".
...
Now we could fit this to the training data, look at the model
